...
class Application(tk.Frame):
    ...
    def create_widgets(self):
        ...
        self.ipPort.insert(tk.END, '10.0.2.103:60003')
...
def tryToConnect():
    ...
    if g_bConnected == False:
        try: 
            IPAddress = g_app.ipPort.get().split(":")
            g_sock.connect((str(IPAddress[0]), int(IPAddress[1])))
            printToMessages("hallllo")
            g_bConnected = True
            g_app.connectButton['text'] = 'disconnect'  
        except: 
            printToMessages("Can't Connect")
    elif g_bConnected == True: 
        printToMessages("You are already connected")
    # your code here
...
def sendMessage(master):
    ...
    if g_bConnected == True:
        try: 
            ...
            printToMessages("Error --> " + str(e))
    else:
        printToMessages("You need to be connected now.")
        # tryToConnect()
...
def pollMessages():
    ...
    if g_bConnected == True:
        ...
        try:
            ...
            g_sock.sendall(bytearray(message, "acsii"))
        except: 
            pass
    # your code here
